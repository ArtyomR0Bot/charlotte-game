...
func action_move_sideway():
	if state == IDLE:
		state = RUNNING
	$Animation.change()


func action_stop_moving():
	if state == DASHING:
		action_stop_dashing()
	move_speed.x = 0
	if state == RUNNING:
...
func action_action_a():
	if move_speed.x != 0:
		action_dash()


func action_stop_action_a():
	if state == DASHING:
		action_stop_dashing()
